...
export default function CalculatorClient({ fxRate, fxStatus, fxUpdatedAt }: CalculatorClientProps) {
  ...
  const updatedAtLabel = formatUpdatedAt(fxUpdatedAt)
        ...
        <SectionCard title="Inputs">
          <div className="grid gap-4">
            <div>
              ...
              {moneda === 'Bs' && (
                <div className="mt-2 rounded-2xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-white/50">
                  <div>Tasa BCV: 1$ ≈ {Math.round(fx)} Bs</div>
                  <div>Estado: {fxStatus}</div>
                  <div>Actualizada: {updatedAtLabel || 'No disponible'}</div>
                </div>
              )}
            </div>
        ...
        <SectionCard title="Resultado">
          <div className="grid gap-3 text-sm">
            <div className="flex justify-between">
              <span className="text-white/55">Área aprox</span>
              <span>{result.area} dm²</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/55">Horas estimadas</span>
              <span>{result.hours} h</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/55">Mano de obra</span>
              <span>
                {currencySymbol} {displayLabor}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/55">Subtotal</span>
              <span>
                {currencySymbol} {displaySubtotal}
              </span>
            </div>
